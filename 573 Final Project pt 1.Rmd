...
x = data.matrix(cpshealth[ c("RACE", "HISPAN", "FEMALE", "MARST", "METRO", "Education", "HHINCOME", "AGE", "VETSTAT", "FAMSIZE", "LABFORCE", "bornabroad", "UHRSWORKT", "indmove", "lostjob", "ANYCOVNW")])
y = cpshealth$HEALTH
...
# OLS
```{r}
OLS = lm(y ~ factor(RACE) + HISPAN + FEMALE + MARST + METRO + Education + factor(STATEFIP) + HHINCOME + AGE+ VETSTAT + FAMSIZE + LABFORCE+ bornabroad + UHRSWORKT + indmove + lostjob + ANYCOVNW, data = cpshealth)
summary(OLS)

ols_result = tidy(OLS)
write.table(ols_result, file = "olstab.txt", sep = ",", quote = FALSE, row.names = F)
...
knn.pred <- knn(knn.train, knn.test, train.health, k = 1000)
