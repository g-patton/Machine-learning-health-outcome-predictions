...
countypopdata$GeoFIPS = str_sub(countypopdata$GeoFIPS,2,-
                                  2)
...
dfhealth_numeric = cpshealth %>%
  select(HISPAN, FEMALE, MARST, METRO, Education, HHINCOME, AGE, VETSTAT, FAMSI
         ZE, LABFORCE, bornabroad, UHRSWORKT, indmove, lostjob, ANYCOVNW, HEALTH )
...
mean(ols_summ$residuals^2)

##prediction

data_ols = data.frame(pred = predict(OLS), actual = cpshealth$HEALTH)

head(data_ols)
mean((data_ols$actual - data_ols$pred)^2)
...
#reports lowest at 0.0083   this is also MSE
...
y_predicted_ridge = predict(ridge_m, s= ridge.model$lambda.min
, newx = x )

SST_ridge = sum((y - mean(y))^2)
SSE_ridge = sum((y_predicted_ridge - y)^2)
rsq_ridge = 1- SSE_ridge/SST_ridge
rsq_ridge
# 0.1975982

y_predicted_best.ridge = predict(new_ridge, s= ridge.model$lambda.min
, newx = x )

SST_ridge = sum((y - mean(y))^2)
SSE_ridge2 = sum((y_predicted_best.ridge - y)^2)
rsq_ridge2 = 1- SSE_ridge2/SST_ridge
rsq_ridge2
...
print(lowest_lambda_lasso)
#0.0002177337

best.lasso = glmnet(x,y, alpha=1, lambda = lowest_lambda_lasso)

#checking coef's for lasso
coef(best.lasso)


y_predicted_lasso = predict(lasso.base, s= lasso.model$lambda.min
, newx = x )

SST_lasso = sum((y - mean(y))^2)
SSE_lasso = sum((y_predicted_lasso - y)^2)
rsq_lasso = 1- SSE_lasso/SST_lasso
rsq_lasso
#0.1976951

y_predicted_best.lasso = predict(best.lasso, s= lasso.model$lambda.min
, newx = x )

SST_lasso = sum((y - mean(y))^2)
SSE_lasso2 = sum((y_predicted_best.lasso - y)^2)
rsq_lasso2 = 1- SSE_lasso2/SST_lasso
rsq_lasso2
...
```{r}
pca.results <- prcomp(dfhealth_numeric, center = TRUE,scale. = TRUE)
pca.results
autoplot(pca.results, loadings = TRUE)

pca.results$rotation = -1*pca.results$rotation


pca.results$x = -1*pca.results$x
head(pca.results$x)

pca.results$sdev^2/ sum(pca.results$sdev^2)

#this output by number is the percentage of variance explained by each principle component
...
train_health <- createDataPartition(dfhealth$HEALTH, 
                                         p = .8,
                                  list = FALSE,
                                  times = 1)
vector_train_index =c(train_health)
...
plot(elastic.model, ylab = "RMSE")
elastic.model

mean(elastic.model$resample$RMSE)
...
predictions.train = predict(elastic.model, train_elastic)

predict_test = predict(elastic.model, valid_elastic)

mean(elastic.model$resample$Rsquared)


elastic.mse = (mean(elastic.model$resample$Rsquared))^2
...
#linear:12000
#radial: 6000
#poly: 1200-1300 is smallest where instances of all variables are exhibited. even reducing it this small would not allow the laptop to run it within 4-5 hours

### ############################################################# copy from here to change sample size
df_svm = sample_n(df_svm_full, 12000)
...
################################################################## to here to change sample size
################################################################### and paste it above the model you want to use
...
mean(svm_linear$results$Kappa[2:20])
...
#could not run this test due to computational and time constraints, even after reducing sample sizes down to the 1000's. 
# ######################################
